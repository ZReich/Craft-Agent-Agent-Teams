...
export class SessionManager {
  ...
  private startTeamHealthMonitoring(teamId: string, leadSessionId: string): void {
    ...
    const statusInterval = setInterval(() => {
      ...
      const rl = teamManager.getReviewLoop(teamId)
      if (rl) {
  ...
  private setupYoloOrchestrator(teamId: string, leadSessionId: string, workingDirectory: string): YoloOrchestrator | null {
    ...
    const reviewLoop = teamManager.getReviewLoop(teamId)
    if (!reviewLoop) {
  ...
  async createTeammateSession(params: {
    ...
    if (qgConfig.enabled && !teamManager.getReviewLoop(resolvedTeamId)) {
      ...
      // Implements REQ-003: scope review loops per team to avoid config leakage.
      teamManager.setReviewLoop(reviewLoop, resolvedTeamId)
    ...
    const agentTeamsConfig = workspaceConfig?.agentTeams as Record<string, unknown> | undefined
    const settingsConfig = (workspaceConfig as any)?.settings as Record<string, unknown> | undefined
    const uxDesignPreferOpus = (
      (agentTeamsConfig?.uxDesignPreferOpus as boolean | undefined)
      ?? (settingsConfig?.agentTeamsUxDesignPreferOpus as boolean | undefined)
      ?? true
    )
    ...
    // Implements REQ-AUDIT-007: allow users to disable UX Opus preference from settings.
    const effectiveModelOverride = (routing.domain === 'ux_design' && !uxDesignPreferOpus)
      ? undefined
      : routing.modelOverride
    if (routing.domain === 'ux_design' && routing.modelOverride && !effectiveModelOverride) {
      sessionLog.info(
        `[AgentTeams] UX/Design Opus preference disabled by settings; ` +
        `using configured model resolution instead of "${routing.modelOverride}".`
      )
    }

    // Implements REQ-001/REQ-005: honor workspace role model defaults, with optional UX Opus preference.
    let teammateModel = effectiveModelOverride ?? configuredModel ?? normalizedRequestedModel ?? parent.model ?? workspaceConfig?.defaults?.model ?? DEFAULT_MODEL
    ...
    if (effectiveModelOverride && configuredModel && effectiveModelOverride !== configuredModel) {
      sessionLog.info(
        `[AgentTeams] Routing policy enforced model override for domain=${routing.domain}: ` +
        `"${configuredModel}" → "${effectiveModelOverride}" (${routing.reason})`
      )
    }

    // Implements REQ-004: choose a provider/connection compatible with the resolved model
    let teammateConnectionSlug = resolveConnectionForModel({
      model: teammateModel,
    ...
    if (!teammateConnectionSlug) {
      // Implements REQ-AUDIT-006: UX fallback policy when Opus is unavailable.
      // Fallback order: user-configured Codex model (if any) -> GPT-5.3 Codex Spark -> GPT-5.3 Codex.
      if (routing.domain === 'ux_design' && effectiveModelOverride === 'claude-opus-4-6') {
        const fallbackCandidates = [
          ...
          Boolean(candidate && candidate !== 'claude-opus-4-6' && isCodexModel(candidate))
        )

        const uniqueCandidates = [...new Set(fallbackCandidates)]
        for (const candidate of uniqueCandidates) {
          const candidateConnection = resolveConnectionForModel({
            model: candidate,
            parentConnectionSlug: params.llmConnection ?? parent.llmConnection,
            workspaceDefaultSlug: workspaceConfig?.defaults?.defaultLlmConnection,
          })
          if (!candidateConnection) continue
          const conn = getLlmConnection(candidateConnection)
          if (!conn || !connectionSupportsModel(conn, candidate)) continue

          teammateModel = candidate
          teammateConnectionSlug = candidateConnection
          sessionLog.warn(
            `[AgentTeams] UX/Design Opus override unavailable; falling back to "${candidate}" (${routing.reason})`
          )
          break
        }
        ...
        if (!teammateConnectionSlug) {
          throw new Error(
            `UX/Design routing prefers "claude-opus-4-6", but no compatible connection is configured and no Codex fallback was found. ` +
            `Configure a connection for Opus 4.6, gpt-5.3-codex-spark, or gpt-5.3-codex.`
          )
        }
      }
    ...
    // Implements REQ-AUDIT-003: use canonical agentTeams.* config shape with legacy fallback.
    const presetId = (
      agentTeamsConfig?.modelPreset
      ?? settingsConfig?.agentTeamsModelPreset
    ) as string | undefined
    const thinkingKey = `${teammateRole}Thinking`
    const legacyThinkingKey = `agentTeams${teammateRole.charAt(0).toUpperCase() + teammateRole.slice(1)}Thinking`
    const customThinkingOverride = presetId === 'custom'
      ? (
        (agentTeamsConfig?.[thinkingKey] as boolean | undefined)
        ?? (settingsConfig?.[legacyThinkingKey] as boolean | undefined)
      )
      : undefined
    // Implements REQ-AUDIT-006: UX/Design should always run with thinking ON for quality.
    const shouldEnableThinking = routing.domain === 'ux_design'
      ? true
      : resolveThinkingForRole(presetId, teammateRole, customThinkingOverride)
    const teammateThinkingLevel: ThinkingLevel = shouldEnableThinking ? 'think' : DEFAULT_THINKING_LEVEL

    // Implements REQ-AUDIT-003: emit complete spawn decision telemetry for role/model/preset/thinking.
    sessionLog.info(
      `[AgentTeams] Resolved teammate spawn: name=${teammateDisplayName}${isAutoGenerated ? ' (auto-generated)' : ''} ` +
      `role=${teammateRole} model=${teammateModel} preset=${presetId ?? 'unset'} ` +
      `thinking=${shouldEnableThinking ? 'on' : 'off'} level=${teammateThinkingLevel} ` +
      `uxPreferOpus=${uxDesignPreferOpus ? 'on' : 'off'} (${routing.reason})`
    )
  ...
  cleanup(): void {
    ...
    for (const reviewLoop of teamManager.getAllReviewLoops()) {
